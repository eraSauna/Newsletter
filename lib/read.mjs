...
const SYSTEM = `Je leest en verifieert bronnen voor eräSauna (verplaatsbare outdoor drop-in sauna's; markten NL/BE/FR/DE/ES/PT, trendwatch UK/DK). Beoordeel per bron op basis van de meegeleverde paginatekst. Verzin niets; als iets niet in de tekst staat, laat het leeg of zet relevant=false. Onderscheid plan/voorstel van definitief. Alleen relevant voor een drop-in outdoor saunaconcept; geen indoor spa/thermen. Een bron telt alleen als relevant met een concrete gedateerde ontwikkeling: opening, besluit, vergunning, tender/aanbesteding, start, uitbreiding, pilot, aankondiging of prijswijziging. Tijdloze/evergreen beschrijvingen van al bestaande faciliteiten zonder zo'n ontwikkeling → relevant=false, ook als de locatie op zich zou passen. Ontwikkelingen vanaf het voorjaar van 2026 tellen als nieuw en relevant, ook als ze niet van deze week zijn. Marktrapporten, 'industry report/market trends', algemene how-to/blogs, productpagina's/webshops en items buiten NL/BE/FR/DE/ES/PT (of UK/DK voor trendwatch) zijn niet relevant (relevant=false).`;
 ...
 */
export async function readItems({ items, model, ledger, concurrency = 4, focus = "" }) {
...
Beoordeel deze bron voor eräSauna en geef het JSON-object volgens het schema. block = het best passende blok. Geef een locatiescore 0-100 en prioriteit A/B/C.`;
    ...
    try {
      const { text } = await geminiCall({ model, system: `${SYSTEM}\nEditiefocus (leidend): ${focus}`, prompt, schema: SCHEMA, ledger, label: "read" });
