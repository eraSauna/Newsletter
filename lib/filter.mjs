...
const SYSTEM = `Je bent een grove voorfilter voor eräSauna (verplaatsbare outdoor drop-in sauna's; markten NL/BE/FR/DE/ES/PT + UK/DK trendwatch). Je beoordeelt alleen titel + snippet, niet de volledige tekst. Doel: duidelijk irrelevante of dubbele items eruit, de rest doorlaten. Wees ruim: bij twijfel doorlaten. Irrelevant = geen enkele link met water/natuur/recreatie/horeca-locaties, zwemmen/waterrecreatie, outdoor sauna, of directe drop-in concurrenten. Indoor spa/thermen/hotelsauna zonder outdoor drop-in = irrelevant. Ook irrelevant: tijdloze/evergreen beschrijvingen van al langer bestaande locaties of faciliteiten zonder concrete recente ontwikkeling (opening, besluit, vergunning, tender, uitbreiding), marktrapporten en 'industry report/market trends', algemene how-to/blog/listicle, productpagina's/webshops, en items buiten NL/BE/FR/DE/ES/PT/UK/DK. Let op: een gedateerde ontwikkeling sinds het voorjaar van 2026 is nieuw en telt mee — die niet afwijzen omdat hij niet van deze week is.`;

/**
 * @returns {Promise<{kept: object[], rejected: object[]}>}
 */
export async function filterCandidates({ candidates, model, ledger, focus = "" }) {
...
${list}`;
  ...
  const { text } = await geminiCall({ model, system: `${SYSTEM}\nEditiefocus (leidend): ${focus}`, prompt, ledger, label: "filter" });
