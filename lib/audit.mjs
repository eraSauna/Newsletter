...
const SYSTEM = `Je controleert of een voorfilter een relevante lead voor eräSauna ten onrechte heeft weggegooid (verplaatsbare outdoor drop-in sauna's; NL/BE/FR/DE/ES/PT + UK/DK). Wees streng: markeer alleen als "gemist" wanneer het item duidelijk een kans, partner, relevante regelgeving of directe concurrent-beweging is.`;
 ...
 */
export async function auditRejected({ rejected, size, model, ledger, concurrency = 3, focus = "" }) {
...
Is dit ten onrechte weggegooid? Antwoord met JSON: {"missed": true/false, "reason": "kort", "block": "..."}. Geen tekst buiten de JSON.`;
    try {
      const { text } = await geminiCall({ model, system: `${SYSTEM}\nEditiefocus (leidend): ${focus}`, prompt, ledger, label: "audit" });
